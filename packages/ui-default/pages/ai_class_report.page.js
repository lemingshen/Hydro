...
const STYLE = [
  '.acr-btn { display: inline-flex; align-items: center; gap: 6px; border: none; border-radius: 16px; padding: 6px 16px; font-size: 13px; color: #fff; cursor: pointer; background: linear-gradient(90deg, #4c6ef5, #845ef7); box-shadow: 0 2px 10px rgba(76,110,245,.35); margin-left: 10px; vertical-align: middle; }',
  '.acr-btn:hover { filter: brightness(1.08); }',
  '@keyframes acrMaskIn { from { opacity: 0; } }',
  '@keyframes acrPopIn { from { opacity: 0; transform: translateY(16px) scale(.95); } 70% { transform: translateY(-2px) scale(1.005); } to { opacity: 1; transform: none; } }',
  '.acr-mask { position: fixed; inset: 0; z-index: 3300; background: rgba(0,0,0,.45); display: flex; align-items: center; justify-content: center; padding: 20px; animation: acrMaskIn .2s ease-out; }',
  '.acr { background: #fff; border-radius: 10px; width: 940px; max-width: 96vw; max-height: 92vh; display: flex; flex-direction: column; box-shadow: 0 12px 40px rgba(0,0,0,.3); overflow: hidden; animation: acrPopIn .3s cubic-bezier(.2,.8,.3,1); }',
  ...
  '.acr__head { display: flex; align-items: center; gap: 10px; padding: 11px 18px; background: linear-gradient(90deg, #4c6ef5 0%, #845ef7 55%, #b197fc 100%); color: #fff; flex: 0 0 auto; }',
  ...
  '.acr__pill { background: rgba(255,255,255,.16); border: 1px solid rgba(255,255,255,.75); color: #fff; padding: 3px 14px; font-size: 12px; border-radius: 14px; cursor: pointer; flex: 0 0 auto; }',
  '.acr__pill:hover { background: rgba(255,255,255,.32); }',
  '.acr__pill:disabled { opacity: .6; cursor: default; }',
  '.acr__close { border: none; background: transparent; font-size: 20px; color: rgba(255,255,255,.9); cursor: pointer; padding: 0 6px; line-height: 1; }',
  '.acr__body { padding: 14px 20px; overflow-y: auto; font-size: 13.5px; }',
  '.acr__stats { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 12px; }',
  '.acr__stat { background: #f4f6fb; border: 1px solid #e3e8f4; border-radius: 8px; padding: 6px 12px; font-size: 12.5px; color: #444; }',
  '.acr__stat b { color: #4c6ef5; }',
  '.acr__empty { text-align: center; padding: 26px 16px; color: #777; }',
  '.acr__gen { display: block; margin: 6px auto 14px; border: none; border-radius: 18px; padding: 10px 26px; font-size: 14px; color: #fff; cursor: pointer; background: linear-gradient(90deg, #4c6ef5, #845ef7); box-shadow: 0 2px 12px rgba(76,110,245,.4); }',
  '.acr__gen:hover { filter: brightness(1.08); }',
  '.acr__wait { display: flex; align-items: center; gap: 14px; padding: 22px 14px; }',
  '@keyframes acr-spin { to { transform: rotate(360deg); } }',
  '.acr__spinner { width: 28px; height: 28px; border: 3px solid #d8e0f8; border-top-color: #4c6ef5; border-radius: 50%; animation: acr-spin .8s linear infinite; flex: 0 0 auto; }',
  '.acr__report { line-height: 1.65; }',
  ...
  '.acr__report pre { background: #f8f7fc; border: 1px solid #e9e4f5; border-radius: 6px; padding: 10px 12px; overflow-x: auto; font-size: 12.5px; line-height: 1.5; }',
  '.acr__report code { background: #f1edfa; border-radius: 3px; padding: 1px 5px; font-size: 12.5px; color: #5f3dc4; }',
  '.acr__report pre code { background: none; padding: 0; color: inherit; }',
  '.acr__report blockquote { margin: 8px 0; padding: 6px 12px; border-left: 3px solid #b197fc; background: #f7f4ff; color: #555; }',
  ...
  '.acr__chartcard { background: #fbfcff; border: 1px solid #e6eaf5; border-radius: 8px; padding: 8px 12px 6px; }',
  '.acr__chartcard h4 { margin: 0 0 4px; font-size: 12.5px; color: #4c6ef5; }',
  '.acr__chartcard canvas { display: block; }',
  // dark theme
  '.pta-dark .acr__chartcard { background: #262b31; border-color: #333a41; }',
  '.pta-dark .acr__chartcard h4 { color: #91a7ff; }',
