...
const STYLE = [
  /* ------------------------------ card shell ------------------------------ */
  '.sbt { border: 1px solid var(--pta-violet-line); border-radius: var(--pta-radius); margin: 16px 0; background: var(--pta-card); color: var(--pta-ink); overflow: hidden; box-shadow: var(--pta-shadow); animation: ptaFadeUp .28s var(--pta-ease) backwards; }',
  '.sbt__head { display: flex; align-items: center; gap: 10px; padding: 12px 18px; background: linear-gradient(100deg, #7048e8 0%, #845ef7 55%, #b197fc 100%); background-size: 220% 100%; animation: ptaSheen 10s ease infinite; color: #fff; }',
  ...
  /* -------------------------------- buttons ------------------------------- */
  '.sbt__btn { border: none; border-radius: 999px; padding: 8px 22px; font-size: 13px; color: #fff; cursor: pointer; background: linear-gradient(90deg, #7048e8, #9775fa); box-shadow: 0 3px 12px rgba(112,72,232,.35); transition: filter .12s ease, transform .12s var(--pta-ease), box-shadow .12s ease; }',
  '.sbt__btn:hover { filter: brightness(1.08); transform: translateY(-1px); box-shadow: 0 6px 16px rgba(112,72,232,.4); }',
  '.sbt__btn:disabled { opacity: .55; cursor: default; transform: none; }',
  '.sbt__btn:focus-visible { outline: 2px solid var(--pta-violet-2); outline-offset: 2px; }',
  '.sbt__btn--ghost { background: var(--pta-card); color: var(--pta-violet-text); border: 1px solid var(--pta-violet-line); box-shadow: none; }',
  ...
  '.sbt__file { display: flex; align-items: center; gap: 10px; padding: 8px 12px; border: 1px solid var(--pta-violet-line); border-radius: 10px; background: var(--pta-card); transition: box-shadow .12s, border-color .12s; animation: ptaFadeUp .24s var(--pta-ease) backwards; }',
  '.sbt__file:hover { border-color: var(--pta-violet-2); box-shadow: 0 3px 12px rgba(95,61,196,.10); }',
  '.sbt__fic { font-size: 16px; flex: 0 0 auto; }',
  '.sbt__file a { color: var(--pta-violet-text); text-decoration: none; font-weight: 600; word-break: break-all; }',
  '.sbt__file a:hover { text-decoration: underline; }',
  '.sbt__fmeta { margin-left: auto; color: var(--pta-ink-faint); font-size: 11.5px; white-space: nowrap; flex: 0 0 auto; }',
  '.sbt__del { border: 1px solid transparent; background: transparent; color: var(--pta-crimson); cursor: pointer; font-size: 14px; line-height: 1; padding: 4px 7px; border-radius: 8px; flex: 0 0 auto; transition: background .15s ease, border-color .15s ease; }',
  '.sbt__del:hover { background: var(--pta-bad-soft); border-color: var(--pta-bad-line); }',
  ...
  '.sbtm-mask { position: fixed; inset: 0; z-index: 3300; background: rgba(10,14,22,.5); backdrop-filter: blur(4px); -webkit-backdrop-filter: blur(4px); display: flex; align-items: center; justify-content: center; padding: 20px; animation: ptaFadeIn .2s ease-out; }',
  '.sbtm { width: 860px; max-width: 94vw; max-height: 90vh; display: flex; flex-direction: column; margin: 0; animation: ptaScaleIn .25s var(--pta-ease); }',
  '.sbtm .sbt__body { overflow-y: auto; }',
  '.sbtm__close { border: none; background: transparent; color: rgba(255,255,255,.9); font-size: 20px; cursor: pointer; padding: 0 4px; line-height: 1; flex: 0 0 auto; transition: transform .2s var(--pta-ease); }',
  '.sbtm__close:hover { color: #fff; transform: rotate(90deg); }',
